...
Asimismo, se incorporó una lógica de filtrado en la etapa de verificación para garantizar la validez de los resultados y optimizar la salida.
...
def amigos(MAX):
    ...
    # Inicializacion:
    # - El 0 no tiene divisores (0).
    # - El 1 no divisores propios
    # - Del 2 en adelante, se precarga 1 (el divisor universal propio).
    suma_divisores = [0, 0] + [1] * (MAX - 1)
    ...
    # Verificacion de pares: Identifica pares amigos
    for numero in range(MAX + 1):
        posible_amigo = suma_divisores[numero]

        # Validacion de reciprocidad:
        # 1. posible_amigo <= MAX: Evita IndexError si la suma 
        #    excede el limite.
        # 2. numero < posible_amigo: Evita duplicados y excluye
        #    numeros perfectos.
        # 3. suma_divisores[posible_amigo] == numero: Confirma 
        #    la amistad.
        if (
            posible_amigo <= MAX
            and numero < posible_amigo
            and suma_divisores[posible_amigo] == numero
        ):
...
Como se observa en la validación de reciprocidad, la condición $\text{numero} < \text{posible\_amigo}$ cumple una doble función: por un lado, descarta los \textbf{números perfectos} (donde la suma es igual al número) y, por otro, actúa como un sistema de \textbf{control de duplicados}. Al exigir una relación de orden, el algoritmo solo informa el par cuando encuentra al menor de los dos componentes, evitando procesar y mostrar la relación inversa (ej. detecta $(220, 284)$ pero ignora el caso cuando el iterador llega a $284$).
...
\begin{itemize}
	...
	\item \textbf{Cuerpo del Algoritmo (Reparto de Divisores):}
	      Esta es la parte central. El bucle externo recorre los divisores \texttt{divisor} desde $2$ hasta \texttt{MAX/2}. Por cada divisor, el bucle interno realiza una cantidad de sumas igual a la cantidad de múltiplos de \texttt{divisor} que entran en el rango MAX.
	      \begin{itemize}
		      \item Para \texttt{divisor} = 2, se realizan $n/2$ operaciones.
		      \item Para \texttt{divisor} = 3, se realizan $n/3$ operaciones.
		      \item Para \texttt{divisor} = 4, se realizan $n/4$ operaciones.
		      \item ... hasta \texttt{MAX/2}, donde se realizan $2$ operaciones.
	      \end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Indexación Booleana:} En lugar de recorrer los $n$ elementos buscando candidatos con un \texttt{if} tradicional, se utiliza la expresión: \\
	      \colorbox{gray!10}{\lstinline[language=Python]|(tabla_sumas > numeros) & (tabla_sumas <= MAX)|}.\\
	      Esta operación genera una "máscara" que filtra el arreglo a nivel de hardware, permitiendo que:
	      \begin{itemize}
		      \item Se optimice el procesamiento: El intérprete de Python solo recibe los índices con alta probabilidad de éxito, reduciendo drásticamente las comparaciones costosas.
		      \item Filtrado de casos triviales: Al exigir que los valores en \texttt{tabla\_sumas} sean mayores a los de \texttt{numeros}, se descartan automáticamente los números perfectos (donde la suma es igual al número) y se garantiza la unicidad de los pares.
          \end{itemize}
\end{itemize}
